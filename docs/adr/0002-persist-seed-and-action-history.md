...
The computer opponent lives outside the engine, so a save also carries the opponent's own RNG state (a single integer) and the Seat the human sits in; neither is engine state, and both are needed to resume exactly.
