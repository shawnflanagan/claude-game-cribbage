...
- Code under `src/engine/` and `src/opponent/` is pure: no `Date`, `Math.random`, timers, or I/O. Randomness comes from the seeded source carried in game state. ESLint enforces this with restricted globals, alongside the import boundaries.
